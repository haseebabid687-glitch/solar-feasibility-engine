...
with tabs[5]:
    ...
    with c1:
        ...
        # Area per MW. A ~600 W module occupies ~2.4 m2, so roughly 250 W/m2 of
        # panel area — about 4,000 m2 (0.4 ha) of pure module per MW before any
        # walkways, setbacks, tilt frames or inter-row spacing are added.
        ha_per_mw_lookup = {
            "Ground-mount – fixed tilt": 2.0,          # incl. inter-row spacing
            "Ground-mount – single-axis tracking": 3.0, # wider rows to avoid shading
            "Rooftop – flat": 1.20,                     # tilt frames + row spacing
            "Rooftop – pitched / optimised": 0.70,      # flush-mounted, follows pitch
            "Mixed (ground-mount + rooftop)": 1.80,
        }
    ...
    with st.expander("View Calculation Methodology — sizing and financials"):
        ...
        st.latex(r"P_{MW} = \frac{A_{effective}}{r_{ha/MW} \times m_{terrain}}")
        st.markdown(f"""
| Term | Value | Source |
|---|---|---|
| `A_cleared` — aviation-cleared area | **{gate2_cleared_ha:,.2f} ha** | Gate 2 output |
| `f_usable` — usable fraction | **{usable_fraction:.0%}** | User input, Gate 6 |
| `A_effective` — effective area | **{effective_ha:,.2f} ha** | {gate2_cleared_ha:,.2f} x {usable_fraction:.2f} |
| `r_ha/MW` — area per MW | **{ha_per_mw_lookup[install_type]:.2f} ha/MW** | {install_type} |
| `m_terrain` — terrain multiplier | **x{terrain_area_mult:.2f}** | {terrain.split('(')[0].strip()} |
| **`P_MW` — indicative capacity** | **{indicative_mw:,.2f} MW** | {effective_ha:,.2f} / ({ha_per_mw_lookup[install_type]:.2f} x {terrain_area_mult:.2f}) |
""")
        if capacity_override:
...
| Land acquisition | {"Airport-owned - no cost" if capex_land == 0 else f"{gate1_available_ha:,.1f} ha acquired"} | **EUR{capex_land:,.0f}** |
...
with tabs[6]:
    ...
    summary_rows = [
        ...
        ("Total site area",                 round(total_site_ha, 1),           "ha"),
        ("Built / operational",             built_pct,                         "%"),
        ("Protected ecology area",          round(green_protected_ha, 1),      "ha"),
        ("Committed to other development",  round(committed_dev_ha, 1),        "ha"),
        ("Gate 1 — available area",         round(gate1_available_ha, 2),      "ha"),
        ("", "", ""),
        ("AVIATION CONSTRAINTS", "", ""),
        ("Glare study completed",           "Yes" if glare_study_done else "No", ""),
        ("Area lost to glare",              round(glare_zones_ha, 1),          "ha"),
        ("OLS mapping completed",           "Yes" if ols_study_done else "No", ""),
        ("Area lost to OLS",                round(ols_eliminated_ha, 1),       "ha"),
        ("Area within RPZ",                 round(rpz_eliminated_ha, 1),       "ha"),
        ("Gate 2 — aviation-cleared area",  round(gate2_cleared_ha, 2),        "ha"),
        ...
        ("Effective area",                  round(effective_ha, 2),            "ha"),
        ("Area per MW",                     round(ha_per_mw, 3),               "ha/MW"),
    ...
    def build_docx() -> bytes:
        ...
        body(f"The site provides {gate1_available_ha:,.1f} ha of land after built, protected and "
             f"committed areas are excluded. Aviation safety constraints reduce this by "
             f"{reduction_pct:.0f}% to {gate2_cleared_ha:,.1f} ha of cleared area. Applying a "
        ...
        kv_table([
            ("Total site area", f"{total_site_ha:,.1f}", "ha"),
            ("Less built / operational", f"-{built_ha:,.1f}", "ha"),
            ("Less protected ecology", f"-{green_protected_ha:,.1f}", "ha"),
            ("Less committed development", f"-{committed_dev_ha:,.1f}", "ha"),
            ("Available area", f"{gate1_available_ha:,.1f}", "ha"),
        ...
        kv_table([
            ...
            ("Area lost to glare", f"-{glare_zones_ha:,.1f}", "ha"),
            ("OLS mapping completed", "Yes" if ols_study_done else "No — restrictions unknown", ""),
            ("Area lost to OLS", f"-{ols_eliminated_ha:,.1f}", "ha"),
            ("Area within runway protection zone", f"-{rpz_eliminated_ha:,.1f}", "ha"),
            ("Aviation-cleared area", f"{gate2_cleared_ha:,.1f}", "ha"),
        ...
        kv_table([
            ("Installation type", install_type, ""),
            ("Effective area after usable fraction", f"{effective_ha:,.2f}", "ha"),
            ("Area per MW", f"{ha_per_mw:,.3f}", "ha/MW"),
        ...
        body(f"Effective area = cleared area x usable fraction = "
             f"{gate2_cleared_ha:,.2f} ha x {usable_fraction:.2f} = {effective_ha:,.2f} ha")
        body(f"Capacity = effective area / (area per MW x terrain multiplier) = "
             f"{effective_ha:,.2f} / ({ha_per_mw_lookup[install_type]:.2f} x "
